...
const loginOne = async(req,res) => {
    ...
    try{
        const {rows} = await conn.query("select customer_id,firstname,lastname,emailid,mobileno from customer where mobileno=$1",[mobileno]);
        if(_.isEmpty(rows)){
...
const loginTwo = async (req,res) => {
    ...
    try{
        const {rows } = await conn.query("select customer_id,firstname,lastname,emailid,mobileno,password from customer where mobileno=$1 and isverified=1",[mobileno]);
        if(_.isEmpty(rows)){
